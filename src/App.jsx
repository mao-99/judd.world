...
import Comparisons from './components/comparisonForm';
...
function App() {
  ...
  let myCounties = ['NEW YORK','DUTCHESS','QUEENS','SULLIVAN','BRONX','ALBANY','MONROE','KINGS','ONONDAGA','WESTCHESTER','NASSAU','SUFFOLK','RICHMOND','FRANKLIN','HERKIMER','ESSEX','CLINTON','SCHENECTADY','ST.LAWRENCE','SARATOGA','WARREN','OTSEGO','WASHINGTON','DELAWARE','CHAUTAUQUA','ERIE','ULSTER','ORANGE','COLUMBIA','GREENE','SCHOHARIE','STEUBEN','ALLEGANY','WYOMING','OSWEGO','ONEIDA','CHENANGO','BROOME','ROCKLAND','CHEMUNG','RENSSELAER','NIAGARA','LIVINGSTON','PUTNAM','MONTGOMERY','CATTARAUGUS','GENESEE','CAYUGA','ORLEANS','TIOGA','MADISON','TOMPKINS','ONTARIO','FULTON','CORTLAND','JEFFERSON','WAYNE','LEWIS','YATES','SENECA','SCHUYLER','HAMILTON']
  myCrimes = myCrimes.filter((element, index, self) => {
    return self.indexOf(element) ===index;
  })
  const [allCrimes, setAllCrimes] = useState(myCrimes);
  const [allCounties, setAllCounties] = useState(myCounties)
  const [crimesAndDegrees, setCrimesAndDegrees] = useState({});
  const [comparisonObject1, setComparisonObject1] = useState({});
  const [comparisonObject2, setComparisonObject2] = useState({});
  const [comparisonObject3, setComparisonObject3] = useState({});
  const [comparisonObject4, setComparisonObject4] = useState({});
  useEffect(()=>{
    const loadDatabase = async () => {
      const {data, error} = await supabase.from('convictionData').select('din, lastName, firstName, sentenceDuration, crimeArray, degreeArray, age, race, multipleOffense, county, status, aggMinSentence, aggMaxSentence').gt('sentenceDuration', 0);
      if (error){
  ...
  if (loading){
    return <h1><img src="/loading.gif" alt="Database Loading" /></h1>
  }
  else{
    return (
      <>
        <Router>
          <div className="body">
            ...
            <Routes>
              ...
              <Route path='/simple' element={<SimpleLookup allCrimes={allCrimes} database={data} allCounties={allCounties} />} />
              ...
              <Route path='/comparison' element={<Comparison database={data}/>}>
                <Route path='/comparison/form' element={<ComparisonForm allCrimes={allCrimes} allCounties={allCounties} setComparisonObject1={setComparisonObject1} setComparisonObject2={setComparisonObject2} setComparisonObject3={setComparisonObject3} setComparisonObject4={setComparisonObject4} />} />
                <Route path='/comparison/plot' element={<ComparisonPlot database={data} object1={comparisonObject1} object2={comparisonObject2} object3={comparisonObject3} object4={comparisonObject4} allCrimes={allCrimes} />} />
              </Route>
            </Routes>
          </div>
        </Router>
